if(NOT TARGET Cinder-Beat)
    # Define ${Cinder-Notifications_PROJECT_ROOT}. ${CMAKE_CURRENT_LIST_DIR} is just the current directory.
    get_filename_component(Cinder-Beat_PROJECT_ROOT "${CMAKE_CURRENT_LIST_DIR}/../.." ABSOLUTE)

    # Define ${CINDER_PATH} as usual.
    get_filename_component(CINDER_PATH "${Cinder-Beat_PROJECT_ROOT}/../.." ABSOLUTE)

    # Make a list of source files and define that to be ${SOURCE_LIST}.
    file(GLOB SOURCE_LIST CONFIGURE_DEPENDS
            "${Cinder-Beat_PROJECT_ROOT}/src/fft/fft.cpp"
            "${Cinder-Beat_PROJECT_ROOT}/src/Beat.cpp"
            )

    # Create the library!
    add_library(Cinder-Beat ${SOURCE_LIST})

    # Add include directories.
    # Notice that `cinderblock.xml` has `<includePath>src</includePath>`.
    # So you need to set `../../src/` to include.
    target_include_directories(Cinder-Beat PUBLIC "${Cinder-Beat_PROJECT_ROOT}/src" )
    target_include_directories(Cinder-Beat SYSTEM BEFORE PUBLIC "${CINDER_PATH}/include" )
    ...
    target_link_libraries(Cinder-Beat PRIVATE cinder)
